...
export class RectangleComponent extends ShapeComponent implements OnInit {
    ...
    resizeShape(resizePosition: MousePosition) {
        console.log('reactangle: resizeShape ', resizePosition);
        if (this.shape instanceof Rectangle) {
            this.shape.width = Math.abs(resizePosition.x - this.shape.originX);
            this.shape.height = Math.abs(resizePosition.y - this.shape.originY);
        }
    }
